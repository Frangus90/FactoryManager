...
export default function MapSettingsPage() {
  ...
  useEffect(() => {
    window.electronAPI.mapSettings.readMapSettings().then(setMapSettings).catch(console.error);
    window.electronAPI.mapSettings.readMapGenSettings().then(setGenSettings).catch(console.error);
  }, []);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setToast('Failed to save map settings.');
    } finally {
      setSaving(false);
    }
  }, [mapSettings, genSettings]);

  if (!mapSettings || !genSettings) {
    return <div className="flex items-center justify-center h-full"><p className="text-factorio-muted">Loading...</p></div>;
  }
    ...
    <div className="h-full flex flex-col overflow-hidden">
      {toast && (
        <div className="toast-success mx-4 mt-4 flex items-center justify-between">
          <span>{toast}</span>
          <button className="text-green-400 hover:text-green-200 ml-4" onClick={() => setToast(null)}>&times;</button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <p className="text-xs text-factorio-muted">
          These settings apply when creating a new save. They do not affect existing saves.
        </p>
      </div>
    </div>
  );
