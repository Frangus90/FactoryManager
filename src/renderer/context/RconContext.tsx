...
export function RconProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    window.electronAPI.rcon.getStatus().then(setRconStatus).catch(() => {});
...
export function useRconContext(): RconContextValue {
  ...
  if (!ctx) throw new Error('useRconContext must be used within RconProvider');
  return ctx;
}
